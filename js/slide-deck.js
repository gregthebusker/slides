 ...
 * @private
 */
SlideDeck.prototype.nextStep_ = function() {
  var slide = this.slides[this.curSlide_];
  var numOfSteps = parseInt(slide.getAttribute('data-number-of-steps'));
  if (numOfSteps > 0) {
    var stepOn = parseInt(slide.getAttribute('data-step-on'));
    if (isNaN(stepOn)) {
      stepOn = 0;
    }
    stepOn+=1;

    if (stepOn <= numOfSteps) {
      slide.setAttribute('data-step-on', stepOn);
      return true;
    }
  }
